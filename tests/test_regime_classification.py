...
def test_space_like_means_learning():
    ...
    print(
        f"Space-like: early={space_early}/{len(early)}, late={space_late}/{len(late)}"
    )
...
def test_extreme_regimes():
    ...
    print(
        f"  Space-like: C={C_space}, S={S_space}, ds²={ds2_space:5d} → {regime_space}"
    )
    print(
        f"  Light-like: C={C_light}, S={S_light}, ds²={ds2_light:5d} → {regime_light}"
    )
